...
```text
full front view of a complete smartphone displaying a virtual AI productivity planner app UI named MODU, entire phone visible from top to bottom, entire app screen visible, not cropped, centered composition, straight-on view, clean futuristic smartphone app UI, minimal premium dashboard layout, dark navy and deep purple background, neon blue and pink highlights, three simple task cards clearly visible, balanced spacing, top header area with MODU wordmark, checklist icons, calendar widget, high contrast, professional app advertisement style, no hands, no extra objects
```
...
- 이후 씬 2~4에서는 새로 확정한 로고와 앱 UI의 색상, 형태, 분위기를 반복 사용해 브랜드 일관성을 유지한다.
...
| 사용 도구 및 목적 | 앞서 신규 생성한 MODU 로고와 앱 UI 기준 이미지를 바탕으로 앱 대시보드 화면을 Ideogram에서 생성한다. 동일한 스타일 키워드를 사용해 씬 간 톤을 일관되게 유지하며, 생성된 정지 이미지에 Runway 또는 CapCut 모션으로 카드가 정렬되는 듯한 움직임을 적용한다. |
| 이미지 생성 프롬프트 원문 | clean futuristic smartphone app UI for an AI productivity planner named MODU, organized dashboard layout, task cards automatically sorted into Today, Study, Work, Personal, calendar widgets, checklist icons, priority tags, dark navy and purple background, neon blue and pink highlights, minimal premium interface, readable UI, balanced spacing, 16:9 app advertisement composition, high contrast, consistent with newly generated MODU logo and app UI style |
...
| 수정 방향 | 스마트폰을 정면 구도로 고정하고, 다크 네이비·딥 퍼플 배경, 네온 블루·핑크 강조색, 프리미엄 앱 광고 스타일을 구체적으로 명시했다. 기존 MODU 로고와 앱 UI가 없는 상태이므로, 먼저 로고·앱 UI 기준 이미지를 새로 생성하고 이후 “consistent with newly generated MODU logo and app UI style”을 추가해 신규 생성한 기준 비주얼과의 일관성을 강화했다. |
...
| 결과 변화 | 앱 실행 장면의 구도가 명확해졌고, 새로 생성할 MODU 로고와 앱 UI가 같은 브랜드 톤으로 연결되도록 개선되었다. |
...
먼저 Ideogram을 통해 가상 브랜드 MODU의 로고와 앱 UI 기준 이미지를 새로 생성한다. 이후 이 기준 비주얼을 바탕으로 씬별 키비주얼을 제작하고, Runway를 활용해 필요한 장면만 짧은 영상으로 변환한다. 이 방식은 영상 생성 크레딧을 절약하면서도 광고에 필요한 움직임을 확보할 수 있는 현실적인 전략이다.
