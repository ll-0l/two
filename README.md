...
이번 제작에서는 **Ideogram, Runway, Suno, CapCut TTS, CapCut**을 활용하여 이미지·영상·오디오·내레이션·편집 과정을 구성한다.
...
|---|---|---|---|---|
| 이미지 생성 | Ideogram | 키비주얼, 앱 UI, 로고 엔딩 이미지 생성 | 네온·사이버펑크 스타일 표현이 우수하고, 특히 로고·슬로건·앱 UI 등 화면 내 텍스트 렌더링 품질이 뛰어나 브랜드 이미지 제작에 적합하기 때문에 사용 | 기존 MODU 로고/앱 UI 이미지 재활용, 필요 시 Google Whisk 또는 ChatGPT 이미지 생성 |
...
- Ideogram 크레딧이 부족할 경우 기존 소스를 재활용하고, 필요한 장면만 최소한으로 재생성한다.
...
| 사용 도구 및 목적 | 로고와 앱 실행 장면을 Ideogram에서 생성한다. 동일한 스타일 키워드와 시드(seed) 값을 고정해 톤을 일관되게 유지하며, 생성된 이미지에 Runway에서 카드가 중앙으로 모이는 모션을 적용한다. |
...
| 사용 도구 및 목적 | 앞서 생성한 앱 실행 이미지를 기준으로 앱 대시보드 UI를 Ideogram에서 생성한다. 동일한 스타일 키워드와 시드(seed) 값을 고정해 씬 간 톤을 일관되게 유지하며, 생성된 정지 이미지에 Runway 또는 CapCut 모션으로 카드가 정렬되는 듯한 움직임을 적용한다. |
...
| 화면 구성 | 중앙에 앞서 Ideogram에서 생성한 MODU 로고 또는 워드마크가 크게 배치되고, 아래에 슬로건 “할 일 폭주? MODU 정리해.”와 엔딩 카피 “나의 하루를 한눈에.”가 순차적으로 등장한다. 배경은 다크 네이비 또는 딥 퍼플이며, 네온 블루와 핑크 글로우를 사용한다. |
| 내레이션 또는 화면 카피 | 나의 하루를 한눈에. |
| 사용 도구 및 목적 | 기존 MODU 로고 이미지를 기준 소스로 사용한다. 필요 시 Ideogram으로 브랜드 엔딩 화면을 만들고, CapCut에서 로고 페이드인, 카피 등장, BGM 마무리 효과를 적용한다. |
...
또한 크레딧 부족과 도구 제한을 고려하여 모든 장면을 고비용 영상 생성으로 만들기보다, **모든 이미지 소스(로고·앱 UI·엔딩 화면 포함)를 Ideogram에서 처음부터 새로 생성한 뒤 Runway에서 필요한 컷만 영상 변환**하는 방식을 선택하였다.  
특히 Ideogram은 텍스트와 UI 렌더링에 강점이 있어, 로고 워드마크와 앱 대시보드 화면을 일관된 품질로 생성할 수 있다. 씬 간 톤을 통일하기 위해 **동일한 스타일 키워드를 반복 적용하고 시드(seed) 값을 고정**하며, 로고는 한 번 확정한 이미지를 각 씬에 재사용해 브랜드 일관성을 확보한다.  
Runway 사용이 어렵거나 크레딧 제한이 발생할 경우에는 CapCut 모션을 대체 도구로 사용해 줌, 패닝, 글리치 전환을 적용하며, 이미지 생성이 제한되는 비상 상황에서만 기존 MODU 소스를 참조 자료로 활용한다.  
