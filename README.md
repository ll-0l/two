...
| 출력 결과 요약 | 씬 1의 혼란 요소가 공식 MODU 로고가 표시된 스마트폰 안으로 빨려 들어가며 디지털 정리가 시작되는 전환 장면을 확보한다. |
...
| 출력 결과 요약 | 씬 2에서 수집된 디지털 입자가 고정된 MODU 앱 UI 안에서 카드 테두리와 체크 요소로 정착하며, 정리된 하루 일정이 완성되는 핵심 장면을 확보한다. |
...
| 영상 변환 프롬프트 원문 | 3-second premium brand end card animation, MODU logo fades in at center, subtle neon blue and pink glow pulse, dark navy and deep purple futuristic digital background, soft city-light reflections, minimal movement, readable logo, no distortion, no extra objects, leave clean space below the logo for CapCut Korean slogan overlay |
...
브랜드의 핵심 메시지인 “흩어진 할 일을 MODU가 모두 정리해준다”를 시각적으로 보여주기 위해, 씬 1에서는 바쁜 일상 속 여러 준비물과 일정이 한꺼번에 몰린 현실적인 혼란 장면을 제시한다. 씬 2에서는 동일한 종이·포스트잇·홀로그램 일정 조각이 공식 MODU 로고만 표시된 스마트폰으로 흡수되고, 씬 3에서는 수집된 디지털 입자가 고정된 카드형 대시보드 안에서 정리된 결과를 보여준다.
