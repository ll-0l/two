...
| 산출물 | 파일명 | 제출 상태 | 비고 |
|---|---|---|---|
| 스토리보드 PDF | `MODU_storyboard_shinuiryeong.pdf` | 변환 예정 | 본 MD 문서를 최종 수정 후 PDF로 변환 |
| 광고 영상 MP4 | `MODU_brand_ad_shinuiryeong.mp4` | 최종 편집 예정 | Google Flow로 생성한 씬 1~4 영상을 CapCut에서 10초 이내로 편집 |
| 작업 원본 MD | `MODU_storyboard_shinuiryeong.md` | 수정 완료 | Google Flow 생성 결과 반영본 |
| AI 생성 소스 폴더 | `MODU_assets/` | 정리 예정 | 업로드된 씬 1~4 Google Flow 영상, BGM, 내레이션 파일 정리 |
...
이번 제작에서는 **Ideogram, ChatGPT 이미지 생성, Google Flow, Suno, CapCut TTS, CapCut**을 활용한다.  
이미지, 영상, 오디오, 내레이션, 편집 도구의 역할을 분리하고, 각 도구가 가진 장점을 활용하여 다음과 같은 제작 흐름을 설계한다.

> 기획 의도 정의 → 로고·앱 UI 기준 이미지 신규 생성 → 씬별 프롬프트 설계 → Ideogram·ChatGPT 이미지 생성 → Google Flow로 씬 1~4 영상 생성 → 오디오·내레이션 생성 → CapCut 통합 편집 → 최종 MP4 출력
...
| 3단계 | 씬별 키비주얼 생성 및 Google Flow 영상 생성 | Ideogram, ChatGPT 이미지 생성, Google Flow | 확정한 로고·앱 UI의 톤을 기준으로 씬 이미지를 만들고, Google Flow에서 씬별 영상을 생성해 광고 컷으로 활용 |
...
| 비디오 생성/변환 | Google Flow | Ideogram 또는 ChatGPT 이미지 생성으로 만든 씬별 기준 이미지를 바탕으로 짧은 광고 영상을 생성 | 이미지·프롬프트 기반으로 장면의 카메라 무빙, 네온 글로우, 디지털 입자, 전환감을 구현하기 위해 사용 | CapCut 모션, KLING, Pika |
...
| 최종 영상 길이 | 10초 이내 |
| 화면 비율 | 16:9 |
| 목표 해상도 | 최종 출력 720p 이상, 가능하면 1080p |
| 실제 생성 조건 | Google Flow 원본 영상은 1280x720, 24fps로 생성되었으며 과제 허용 기준인 720p 이상을 충족 |
| 프레임레이트 | 원본 24fps, 최종 24fps 또는 30fps |
| 비디오 코덱 | H.264 |
| 오디오 코덱 | AAC |
...
최종 광고 영상은 가능하면 **1080p, 1920x1080 해상도**로 출력하는 것을 목표로 한다.  
다만 실제 Google Flow 생성 원본은 모두 **1280x720, 24fps**로 확인되었으므로, 원본 품질을 안정적으로 유지하려면 **720p 출력**이 가장 안전하다. 과제에서 720p 이상을 허용하므로, 최종 제출 기준은 충족된다.
...
- Google Flow 사용이 어렵거나 크레딧이 부족할 경우 CapCut 모션을 대체 도구로 사용한다.
...
현재 씬 1부터 씬 4까지의 영상은 **Google Flow**를 활용해 생성한 상태이다.  
따라서 이후 작업의 핵심은 새 영상을 추가 생성하는 것이 아니라, 생성된 4개 영상을 10초 이내 광고 타임라인에 맞게 선별·컷 편집하고, BGM·내레이션·자막·엔딩 카피를 통합하는 것이다.

| 구분 | 현재 상태 | 다음 처리 |
|---|---|---|
| 씬 1 영상 | Google Flow 생성 완료 | 가장 정신없는 구간을 2초로 컷 편집 |
| 씬 2 영상 | Google Flow 생성 완료 | 흡수 모션이 가장 명확한 구간을 2초로 컷 편집 |
| 씬 3 영상 | Google Flow 생성 완료 | UI가 안정적으로 보이는 구간을 3초로 컷 편집 |
| 씬 4 영상 | Google Flow 생성 완료 | 로고가 안정적으로 보이는 구간을 3초로 컷 편집 |
| BGM | 제작 필요 | Suno 또는 CapCut 오디오로 10초 광고용 BGM 생성 |
| 내레이션 | 제작 필요 | CapCut TTS로 4문장 내레이션 생성 |
| 최종 편집 | 진행 예정 | CapCut에서 10초 이내 MP4로 통합 편집 |
| 스토리보드 PDF | 변환 예정 | 최종 MD 확인 후 PDF로 변환 |

### 8-2. 실제 Google Flow 생성 영상 검토 결과

업로드된 Google Flow 생성 영상 4개를 확인한 결과, 모든 씬은 **1280x720, 24fps, H.264** 형식으로 생성되었다.  
따라서 최종 영상은 과제 허용 기준인 **720p 이상**을 충족하며, CapCut 최종 출력 시 720p 또는 1080p로 내보낼 수 있다. 원본 품질을 과도하게 확대하지 않으려면 720p 출력이 가장 안정적이고, 제출 형식상 더 높은 해상도가 필요하면 1080p로 업스케일 출력할 수 있다.

| 씬 | 업로드 원본 파일명 | 원본 길이 | 해상도 | 프레임레이트 | 코덱 | 최종 사용 길이 | 권장 사용 구간 |
|---|---|---:|---|---:|---|---:|---|
| 씬 1 | `scene01_task_overload_motion.mp4(2).mp4` | 약 3.04초 | 1280x720 | 24fps | H.264 | 2초 | 0.5초~2.5초 |
| 씬 2 | `scene02_modu_launch_motion.mp4(2).mp4` | 8.00초 | 1280x720 | 24fps | H.264 | 2초 | 0.0초~2.0초 또는 1.0초~3.0초 |
| 씬 3 | `scene03_organized_dashboard_motion.mp4(2).mp4` | 약 4.01초 | 1280x720 | 24fps | H.264 | 3초 | 0.0초~3.0초 |
| 씬 4 | `scene04_modu_endcard_motion.mp4(1).mp4` | 약 4.01초 | 1280x720 | 24fps | H.264 | 3초 | 0.0초~3.0초 |

### 8-3. 영상 검토 요약

| 씬 | 검토 요약 | 편집 판단 |
|---|---|---|
| 씬 1 | 네온 조명이 있는 작업 공간에서 인물이 할 일에 압도된 모습과 종이·포스트잇·디지털 카드가 떠다니는 장면이 확인된다. | 문제 제시 역할이 명확하므로 2초로 압축해 사용한다. |
| 씬 2 | 중앙 스마트폰에 MODU 로고가 보이고, 종이·포스트잇·디지털 입자가 화면 중심으로 모이는 장면이 확인된다. | 8초 전체를 쓰지 않고 흡수 모션이 가장 명확한 2초만 선별한다. |
| 씬 3 | MODU 앱 UI가 정면 스마트폰 화면에 표시되고, 입자와 글로우 효과가 UI를 활성화하는 흐름이 확인된다. | UI가 안정적으로 보이는 3초 구간을 사용한다. |
| 씬 4 | MODU 로고가 크게 표시되고, 같은 네온 회로 배경과 입자 효과가 유지된다. | 로고가 가장 선명한 3초 구간을 사용하고, 한글 카피는 CapCut에서 직접 올린다. |

---

## 9. 씬별 스토리보드 및 최종 프롬프트

본 섹션은 씬별 기획 내용과 이미지 생성 프롬프트, Google Flow 영상 생성 프롬프트를 함께 정리한 최종 제작 기준이다.  
...
| 사용 도구 및 목적 | Ideogram으로 현실적인 일상 속 할 일 폭주 키비주얼을 생성하고, Google Flow에서 짧은 줌인·흔들림·소품 움직임 모션을 적용한다. Google Flow 사용이 어렵거나 크레딧이 부족할 경우 CapCut 모션으로 대체한다. |
...
| 사용 도구 및 목적 | Ideogram에 공식 MODU 로고 이미지와 씬 1 레퍼런스 이미지를 함께 첨부해 키비주얼을 생성한다. Google Flow에서는 종이·포스트잇·홀로그램 카드가 스마트폰으로 흡수되는 모션을 적용한다. |
...
| 사용 도구 및 목적 | ChatGPT 이미지 생성 또는 Ideogram에서 공식 MODU 로고와 최종 앱 UI 기준 이미지를 함께 첨부해 고정된 대시보드 키비주얼을 생성한다. Google Flow에서는 UI를 다시 설계하지 않고 카드 글로우, 체크 활성화, 플러스 버튼 펄스와 디지털 입자 정착 모션만 적용한다. |
...
| 사용 도구 및 목적 | 공식 MODU 로고 이미지와 씬 3 영상의 마지막 안정 프레임을 ChatGPT 이미지 생성 또는 Ideogram에 함께 첨부해 엔딩 키비주얼을 생성한다. Google Flow에서는 로고 자체를 다시 만들지 않고 잔여 폰 아우라가 사라지는 효과, 소량의 디지털 입자 이동, 로고 전체의 한 번의 글로우 펄스만 적용한다. 한글 슬로건과 엔딩 카피는 CapCut에서 직접 입력한다. |
...
영상 생성/변환은 Google Flow를 사용하되, 자막·오디오·로고 오버레이·최종 컷 편집은 CapCut에서 진행해 과제의 통합 편집 범위 안에서 완성도를 높인다.
...
## 12. Google Flow 영상 생성 및 최종 편집 계획

영상 생성/변환은 **Google Flow**에서 진행했으며, 최종 통합 편집은 **CapCut**에서 진행한다.  
Google Flow는 씬별 이미지와 프롬프트를 바탕으로 씬 1~4의 짧은 광고 영상을 생성하는 용도로 사용했다. CapCut은 이후 컷 편집, 자막, 로고 오버레이, 오디오 레벨 조정 등 통합 편집 용도로 사용한다.
...
|---|---:|---:|---|
| `scene01_task_overload_motion.mp4(2).mp4` | 약 3.04초 | 2초 | 인물 주변의 종이, 포스트잇, 홀로그램 카드가 가장 역동적으로 떠다니는 0.5초~2.5초 구간 사용 |
| `scene02_modu_launch_motion.mp4(2).mp4` | 8.00초 | 2초 | 스마트폰으로 종이·카드가 빨려 들어가며 입자 덩어리가 형성되는 0.0초~2.0초 또는 1.0초~3.0초 구간 사용 |
| `scene03_organized_dashboard_motion.mp4(2).mp4` | 약 4.01초 | 3초 | UI가 변형되지 않고 캘린더·일정 카드·플러스 버튼이 안정적으로 활성화되는 0.0초~3.0초 구간 사용 |
| `scene04_modu_endcard_motion.mp4(1).mp4` | 약 4.01초 | 3초 | 잔여 폰 아우라 소멸 → 공식 로고 글로우 → 엔딩 카피 고정 순서가 보이는 0.0초~3.0초 구간 사용 |
...
업로드된 Google Flow 원본 파일명에는 괄호가 포함되어 있으므로, CapCut 편집 전에는 아래처럼 간단한 이름으로 복사해 정리하는 것을 권장한다.

| 현재 업로드 파일명 | 편집용 권장 파일명 |
|---|---|
| `scene01_task_overload_motion.mp4(2).mp4` | `scene01_task_overload_motion.mp4` |
| `scene02_modu_launch_motion.mp4(2).mp4` | `scene02_modu_launch_motion.mp4` |
| `scene03_organized_dashboard_motion.mp4(2).mp4` | `scene03_organized_dashboard_motion.mp4` |
| `scene04_modu_endcard_motion.mp4(1).mp4` | `scene04_modu_endcard_motion.mp4` |

이렇게 정리하면 CapCut 프로젝트, 스토리보드 문서, 최종 제출 파일명의 일관성을 유지하기 쉽다.
...
먼저 Ideogram을 통해 가상 브랜드 MODU의 로고와 앱 UI 기준 이미지를 새로 생성한다. 특히 앱 UI 기준 이미지는 `2026 7월`, `오늘`, 주간 캘린더, `기상`, `아침 운동`, `회의`, `아침 식사` 일정 카드가 보이도록 구체화한다. 이후 이 기준 비주얼을 바탕으로 씬별 키비주얼을 제작하고, Google Flow를 활용해 씬별 장면을 짧은 영상으로 생성한다. 이 방식은 영상 생성 크레딧을 절약하면서도 광고에 필요한 움직임과 실제 앱 광고 같은 구체성을 확보할 수 있는 현실적인 전략이다.
...
| Google Flow 원본 영상이 최종 타임라인보다 길거나 짧음 | 각 씬 생성 길이가 2초·3초 타임라인과 다를 수 있음 | CapCut에서 핵심 동작이 가장 잘 보이는 구간만 선별해 2초·2초·3초·3초로 컷 편집 |
...
| 영상 생성 크레딧 부족 | Google Flow 등 영상 생성 도구의 무료 제한 | 씬 수를 4개로 제한하고, CapCut 모션으로 대체 |
...
| Google Flow 원본 영상이 720p 이상인가? | 완료: 1280x720 확인 |
...
| 실제 생성 결과 요약과 파일명을 최종 파일 기준으로 업데이트했는가? | 일부 완료: 씬 1~4 원본 영상 정보 반영, 최종 MP4 출력 후 최종값 입력 필요 |
...
| 실제 사용한 영상 생성/변환 도구 | Google Flow로 씬 1~4 영상을 생성했으며, 최종 컷 편집은 CapCut에서 진행했는지 |
...
| 실제 결과 파일명 | Google Flow 원본 영상 파일명과 최종 MP4 저장 파일명이 문서의 파일명 규칙과 일치하는지 |
...
현재 씬 1부터 씬 4까지의 영상 생성은 완료되었으므로, 남은 작업은 **선별 편집 → 오디오 제작 → 최종 출력 → 문서 PDF 변환** 순서로 진행한다.

### 19-1. 1단계: 씬별 베스트 구간 선별

Google Flow에서 생성한 원본 영상은 최종 타임라인 길이보다 길 수 있으므로, 각 영상에서 가장 적합한 구간만 선택한다.

| 씬 | 최종 길이 | 선택 기준 |
|---|---:|---|
| 씬 1 | 2초 | 종이, 포스트잇, 홀로그램 카드가 가장 역동적으로 떠다니는 구간 |
| 씬 2 | 2초 | 혼란 요소가 스마트폰 안으로 빨려 들어가는 모션이 가장 명확한 구간 |
| 씬 3 | 3초 | MODU UI가 흔들리거나 왜곡되지 않고 가장 안정적으로 보이는 구간 |
| 씬 4 | 3초 | MODU 로고가 정확히 보이고 엔딩 카피를 넣을 여백이 확보된 구간 |

### 19-2. 2단계: CapCut 10초 타임라인 구성

CapCut에서 16:9 프로젝트를 만들고, 씬별 영상을 아래 순서로 배치한다.
...
각 컷의 연결부에는 과도한 전환 효과를 쓰기보다, 짧은 네온 플래시·글리치·디졸브 정도만 사용한다.

### 19-3. 3단계: 내레이션 제작

CapCut TTS에서 발랄하고 경쾌한 여성 톤으로 아래 문장을 생성한다.
...
각 문장은 씬 길이에 맞춰 너무 빠르지 않게 배치한다.

### 19-4. 4단계: BGM 및 효과음 삽입

Suno 또는 CapCut 오디오를 활용해 10초 광고용 BGM을 넣는다.  
BGM은 내레이션을 방해하지 않도록 낮게 깔고, 씬 전환 지점에는 짧은 효과음을 추가한다.

| 구간 | 추천 효과음 |
|---|---|
| 씬 1 | 알림음, 진동음, 종이 움직임 |
| 씬 2 | 디지털 흡수음, 네온 플래시 |
| 씬 3 | 체크 완료음, 부드러운 UI 활성화음 |
| 씬 4 | 짧은 브랜드 차임, 로고 글로우음 |

### 19-5. 5단계: 엔딩 카피 직접 입력

씬 4에는 한글 텍스트를 영상 생성 결과물에 의존하지 않고 CapCut에서 직접 넣는다.
...
첫 번째 줄은 슬로건으로 크게, 두 번째 줄은 엔딩 카피로 조금 작게 배치한다.
...
| 길이 | 10초 이내 |
| 화면 비율 | 16:9 |
| 해상도 | 원본 기준 720p 권장, 필요 시 1080p 업스케일 가능 |
| 프레임레이트 | 원본 24fps, 최종 24fps 또는 30fps |
| 비디오 코덱 | H.264 |
| 오디오 코덱 | AAC |

### 19-7. 7단계: 문서 최종 업데이트 및 PDF 변환

최종 MP4를 출력한 뒤, 본 문서의 아래 항목을 실제 결과 기준으로 업데이트한다.
...
업데이트가 끝나면 Markdown 문서를 PDF로 변환해 스토리보드 PDF로 제출한다.
...
1. 업로드된 4개 영상 파일명을 편집용 이름으로 정리한다.
2. CapCut에서 16:9 프로젝트를 만들고 해상도는 720p 또는 1080p로 설정한다.
3. 씬 1은 0.5초~2.5초 구간을 2초로 배치한다.
4. 씬 2는 0.0초~2.0초 구간을 먼저 배치해보고, 흡수 모션이 약하면 1.0초~3.0초로 바꾼다.
5. 씬 3은 0.0초~3.0초 구간을 사용해 UI가 안정적으로 보이게 한다.
6. 씬 4는 0.0초~3.0초 구간을 사용하고, 로고 아래에 한글 슬로건과 엔딩 카피를 직접 넣는다.
7. CapCut TTS로 내레이션을 만들고 각 씬 길이에 맞춰 배치한다.
8. Suno 또는 CapCut 오디오로 BGM을 넣고 내레이션이 묻히지 않게 볼륨을 낮춘다.
9. 최종 영상을 `MODU_brand_ad_shinuiryeong.mp4`로 출력한다.
10. 최종 MP4의 실제 길이, 해상도, 프레임레이트를 문서에 마지막으로 반영한 뒤 PDF로 변환한다.
