...
본 프로젝트는 AI 일정·할 일 정리 플래너 앱 **MODU**의 10초 이내 브랜드 광고를 제작하는 것을 목표로 한다.  
광고는 할 일과 일정이 흩어져 있어 부담을 느끼는 20~30대 취업준비생, 프리랜서, 직장인을 대상으로 하며, MODU가 사용자의 하루를 한눈에 정리해주는 생산성 경험을 짧고 경쾌한 영상으로 전달한다.

이번 제작에서는 **Leonardo AI, Runway, Suno, CapCut TTS, CapCut**을 활용하여 이미지·영상·오디오·내레이션·편집 과정을 구성한다.  
핵심은 단순히 AI 도구를 많이 사용하는 것이 아니라, **기획 의도 → 프롬프트 설계 → 생성 결과 확인 → 수정 → 통합 편집**으로 이어지는 제작 파이프라인을 명확하게 설명하는 것이다.
...
- 전달하고 싶은 감정: 복잡함에서 정리됨으로 바뀌는 시원함
- 기대 효과: 사용자가 “내 할 일도 MODU로 정리하고 싶다”는 인상을 받도록 유도
...
| 전환 | MODU 앱이 실행되며 흩어진 정보가 모이기 시작 |
...
최종 광고 영상은 과제의 최대 인코딩 스펙을 기준으로 **1080p, 1920x1080 해상도**를 목표로 제작한다.  
단, 영상 생성 도구의 크레딧 부족, 무료 플랜 제한, 대기열 문제, 출력 해상도 제한이 발생할 경우 과제에서 허용한 기준에 따라 **720p 이상 해상도**로 제작한다.

이 경우에도 최종 편집 단계에서 화면 비율은 16:9로 통일하고, 네온·사이버펑크 톤앤매너가 흐트러지지 않도록 색감, 자막, 로고 배치를 정리한다.

---

## 5. 사용 도구 목록

이번 프로젝트에서는 이미지, 비디오, 오디오, 음성, 편집 도구를 각각 분리하여 사용한다.  
각 도구는 결과물의 역할에 따라 선택했으며, 영상 생성 도구의 크레딧·대기열·해상도 제한에 대비해 대체 도구도 함께 설정한다.

| 구분 | 실제 사용 도구 | 사용 목적 | 선택 이유 | 대체 도구 |
|---|---|---|---|---|
| 이미지 생성 | Leonardo AI | 키비주얼, 앱 UI, 로고 엔딩 이미지 생성 | 네온·사이버펑크 스타일의 고대비 비주얼과 프리미엄 테크 앱 광고 이미지를 만들기 위해 사용 | 기존 MODU 로고/앱 UI 이미지 재활용, 필요 시 Canva AI 또는 Ideogram |
| 비디오 생성/변환 | Runway | Leonardo AI로 생성한 정지 이미지를 짧은 모션 영상으로 변환 | 이미지 기반 영상 변환 품질이 높고, 광고 컷처럼 카메라 무빙과 디지털 전환감을 줄 수 있기 때문에 사용 | CapCut 모션 |
| 오디오 생성 | Suno | 10초 광고에 어울리는 경쾌한 디지털 BGM 생성 | 생산성 앱 광고에 맞는 밝고 에너지 있는 전자음악 분위기를 만들기 위해 사용 | CapCut 기본 오디오 또는 다른 AI 음악 생성 도구 |
...
| 항목 | 보유 여부 | 활용 계획 |
|---|---|---|
| MODU 로고 이미지 | 있음 | 씬 4 브랜드 각인 화면의 기준 소스로 사용하고, 신규 엔딩 카드 생성 시 로고 스타일 레퍼런스로 활용 |
| MODU 앱 UI 이미지 | 있음 | 씬 2 MODU 실행 장면과 씬 3 일정 정리 장면의 기준 소스로 사용 |
| 영상 문구 | 기존안 그대로 사용 | 씬별 내레이션과 화면 카피에 동일하게 반영 |
| 영상 비율 | 16:9 | 모든 이미지 생성 및 편집 타임라인을 16:9 기준으로 통일 |
| 목표 해상도 | 1080p | 1920x1080 출력을 목표로 하되, 도구 제한 시 720p 이상으로 대체 |
...
- 기존 MODU 로고 이미지와 앱 UI 이미지가 있으므로, 신규 생성 이미지도 기존 로고와 UI의 색감 및 분위기를 기준으로 맞춘다.
- Leonardo AI 토큰이 부족할 경우 기존 소스를 재활용하고, 필요한 장면만 최소한으로 재생성한다.
- 1080p 생성이 어려운 경우 과제 허용 기준에 따라 720p 이상으로 제작한다.

### 7-3. 스타일 일관성 유지 전략

MODU 광고는 로고, 앱 UI, 배경 그래픽이 서로 다른 스타일로 보이면 브랜드 경험이 약해질 수 있다. 따라서 모든 프롬프트에는 다음 스타일 키워드를 반복해서 사용한다.
...
| 사용 도구 및 목적 | Leonardo AI로 혼란스러운 디지털 할 일 폭주 장면을 만들고, Runway에서 짧은 줌인·흔들림 모션을 적용한다. Runway 사용이 어려운 경우 CapCut 모션으로 대체한다. |
| 입력 프롬프트 원문 | chaotic digital task overload scene for a futuristic AI productivity planner app advertisement, many floating task cards, calendar reminders, notification popups, checklist icons, dark navy and deep purple background, neon blue and neon pink highlights, cyberpunk-inspired but clean and readable, youthful digital mood, premium tech branding, high contrast, 16:9 composition, no real brand logos, no human face |
| 출력 결과 요약 | 네온 컬러의 할 일 카드와 알림이 화면에 쏟아지는 문제 제시용 키비주얼 확보 |
...
| 사용 도구 및 목적 | 기존 MODU 앱 UI 이미지를 기준으로 활용한다. 필요 시 Leonardo AI에서 정리된 앱 대시보드 UI를 생성하고, Runway 또는 CapCut 모션으로 카드가 정렬되는 듯한 움직임을 적용한다. |
| 입력 프롬프트 원문 | clean futuristic smartphone app UI for an AI productivity planner named MODU, organized dashboard layout, task cards automatically sorted into Today, Study, Work, Personal, calendar widgets, checklist icons, priority tags, dark navy and purple background, neon blue and pink highlights, minimal premium interface, readable UI, balanced spacing, 16:9 app advertisement composition, high contrast, consistent with existing MODU logo and app UI style |
| 출력 결과 요약 | 흩어진 할 일이 카드형 대시보드로 정리된 해결 장면 확보 |
...
| 시간 | 화면 | 오디오/내레이션 |
|---|---|---|
| 0:00~0:02 | 할 일 카드와 알림이 폭주하는 네온 화면 | 할 일이 너무 많아? |
| 0:02~0:04 | MODU 앱 실행, 흩어진 카드가 중앙으로 모임 | MODU가 모두 정리해. |
| 0:04~0:07 | 일정과 할 일이 카드형 UI로 자동 정리됨 | 일정, 과제, 약속까지 한눈에. |
| 0:07~0:10 | MODU 로고, 슬로건, 엔딩 카피 노출 | 나의 하루를 한눈에. |
...
| 16:9 비율 | 반영 |
| 목표 해상도 1080p 명시 | 반영 |
| 대체 허용 조건 720p 이상 명시 | 반영 |
| 씬별 필수 필드 작성 | 반영 |
| 이미지 생성 AI 사용 계획 | Leonardo AI로 반영 |
| 비디오 생성 또는 변환 AI 사용 계획 | Runway로 반영 |
| 대체 비디오 도구 | CapCut 모션으로 반영 |
| 오디오 생성 AI 사용 계획 | Suno로 반영 |
| 음성 합성 도구 | CapCut TTS로 반영 |
| 최종 편집 도구 | CapCut으로 반영 |
| 프롬프트 수정 전/후 기록 | 반영 |
| 마지막 3초에 브랜드명/로고/슬로건/엔딩 카피 포함 | 반영 |
| 크레딧 부족 대응 전략 작성 | 반영 |
| 대체 도구 작성 | 반영 |
| 기존 로고 및 앱 UI 이미지 활용 계획 | 반영 |
...
또한 크레딧 부족과 도구 제한을 고려하여 모든 장면을 고비용 영상 생성으로 만들기보다, **Leonardo AI 이미지 생성 결과물과 기존 MODU 로고·앱 UI 이미지를 중심으로 구성한 뒤 Runway에서 필요한 컷만 영상 변환**하는 방식을 선택하였다.  
Runway 사용이 어렵거나 크레딧 제한이 발생할 경우에는 CapCut 모션을 대체 도구로 사용해 줌, 패닝, 글리치 전환을 적용한다.  
최종 영상은 1080p를 목표로 제작하되, 생성 도구의 제한이 발생할 경우 과제 허용 기준에 따라 720p 이상으로 제작한다.

이러한 방식은 제한된 제작 환경에서도 브랜드 톤앤매너, 메시지 전달력, 영상 완성도를 균형 있게 확보할 수 있는 현실적인 AI 광고 제작 전략이다.
