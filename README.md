...
> ✅ **최종 수정본 확인:** 이 README에는 아래 두 원문이 포함되어 있습니다.  
> 1) `MODU 로고 생성 프롬프트`  
> 2) `MODU 앱 UI 레퍼런스 이미지 생성 프롬프트`  
> 두 프롬프트는 **2번 항목**에서 확인할 수 있습니다.
...
## 2. 로고와 앱 UI 레퍼런스 이미지 생성 프롬프트

> **필수 확인:** 아래 두 코드 블록은 로고와 최종 앱 UI 레퍼런스 이미지를 만들 때 사용한 생성 프롬프트 원문이다.

씬별 키비주얼과 Google Flow 영상을 만들기 전에 **MODU 워드마크 로고**와 **최종 앱 UI 레퍼런스 이미지**를 먼저 생성했다. 이후 모든 씬에서 이 두 이미지를 기준으로 색상, 로고 형태, 스마트폰 구도와 대시보드 구조를 고정했다.

| 기준 이미지 | 생성 목적 | 결과 파일명 | 이후 활용 |
|---|---|---|---|
| MODU 워드마크 로고 | 브랜드명과 네온 블루·핑크 색상, `U`의 완료 모티프를 고정 | `01_reference/modu_logo_wordmark.png` | 씬 2 스마트폰 상단, 씬 3 앱 헤더, 씬 4 엔딩 로고의 고정 reference |
| MODU 앱 UI 레퍼런스 | 스마트폰 전체 구도, 한국어 캘린더와 일정 카드, 하단 내비게이션을 고정 | `01_reference/modu_app_ui_reference.png` | 씬 3 최종 UI와 씬 2~4의 스마트폰 크기·위치를 통일하는 reference |
...
**출력 결과 요약:** 다크 네이비·딥 퍼플 배경 위에 네온 블루와 핑크가 결합된 `MODU` 워드마크를 확보했다. `U`에 완료·체크를 연상시키는 형태를 적용해 생산성 앱의 정체성을 표현했으며, 이후 로고 reference로 반복 사용했다.

### MODU 앱 UI 레퍼런스 이미지 생성 프롬프트

```text
full front view of a complete smartphone displaying an AI productivity planner app UI, entire phone visible from top to bottom, entire app screen visible, not cropped, centered composition, straight-on view, clean futuristic smartphone app UI for an AI productivity planner, minimal premium dashboard layout, dark navy and deep purple background, neon blue and neon pink glow accents, cyberpunk-inspired but clean and readable, high contrast, premium tech branding, header with the MODU wordmark in neon blue and neon pink, all interface text in Korean except the MODU brand name, bold modern Korean sans-serif typography, smaller readable Korean text, top date label "2026 7월", large main title "오늘", weekly calendar row "월 화 수 목 금 토 일" with dates "4 5 6 7 8 9 10", Saturday "9" highlighted in neon cyan, four stacked schedule cards reading "기상 7:00", "아침 운동 8:00", "회의 9:00", "아침 식사 10:00", subtle icons, check marks, glowing borders, profile avatars, bottom navigation bar with a glowing plus button, entire smartphone fully visible, no hands, no people, no extra objects, no cropped phone, no tilted perspective, no English text except MODU, no unreadable text, no duplicated UI elements, no distorted letters, no watermark
```

**출력 결과 요약:** 스마트폰 전체가 잘리지 않는 정면 구도와 `2026 7월`, `오늘`, 주간 캘린더, 네 개의 일정 카드가 포함된 대시보드 이미지를 확보했다. 생성 이미지에서 한국어가 일부 변형될 가능성은 있었으므로 구조와 색감을 reference로 고정하고, 광고의 핵심 화면 카피는 CapCut에서 직접 입력했다.

> Leonardo.ai처럼 네거티브 프롬프트 입력 칸이 별도로 보이지 않는 환경에서는 `no hands`, `no cropped phone`, `no unreadable text` 등의 금지 조건을 위 프롬프트 마지막에 함께 입력했다.
...
| 기준 이미지 생성 프롬프트     | MODU 로고와 앱 UI 레퍼런스 생성 프롬프트·결과 파일명·활용 목적 포함         | 완료 |
